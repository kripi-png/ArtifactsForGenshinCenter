import { removeAllArtifacts } from "../artifactRenderer.js";
...
const hideAllArtifactsToggle = function () {
  const checkbox_element = document.querySelector(
    "#toggleVisibilityCheckboxPath",
  );
...
export const ExtensionSettingsSection = () => {
  const STORE_LINK =
    "https://chrome.google.com/webstore/detail/artifacts-for-genshin-cen/jleonalkkhbfeafkmfgofopiadjkalno";
  ...
  EXTENSION_SETTINGS_SECTION.classList.add(
    "PlannerOptions_quickSection__pWVYz",
  );
  ...
  // add callbacks
  EXTENSION_SETTINGS_SECTION.querySelector("#importButton").onclick = () =>
    importArtifactData();
  EXTENSION_SETTINGS_SECTION.querySelector("#exportButton").onclick = () =>
    exportArtifactData();
  EXTENSION_SETTINGS_SECTION.querySelector("button").onclick = () =>
  ...
  EXTENSION_SETTINGS_SECTION.querySelector(
    "." + "Checkbox_checkmarkCheck__ZKcSz",
